...
> Local reproducibility update: start with [LOCAL_REPRODUCTION.md](LOCAL_REPRODUCTION.md).
> It contains a tested offline smoke test, configurable MIMIC preprocessing,
> single-GPU training/inference commands, and an explicit list of artifacts that
> are not included in the public release.

> **DyPH-RAG extension:** the disease-conditioned dynamic patient hypergraph
> framework, CPU smoke test, leakage audits and experiment launcher are
> documented in [DYPHRAG.md](DYPHRAG.md). Existing KARE entry points below are
> preserved.
> The code-level definition of the three innovations is in
> [DYPHRAG_INNOVATIONS.md](DYPHRAG_INNOVATIONS.md).
>
> For an upload-ready server archive and single-file experiment configuration,
> see [SERVER_DEPLOYMENT.md](SERVER_DEPLOYMENT.md). The direct full-matrix entry
> point is `bash scripts/dyphrag_env.sh`.
